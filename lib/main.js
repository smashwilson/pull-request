...
const KEYTAR_USERNAME_KEY = "github"; // Because you can't make an account called "github" ;-)
...
module.exports = {
  ...
  ensureAuthorization: function (callback) {
    ...
    if (atom.config.get("pull-request.useKeyring")) {
      let u = keytar.getPassword(KEYTAR_SERVICE, KEYTAR_USERNAME_KEY);
      if (u) {
        let p = keytar.getPassword(KEYTAR_SERVICE, u);

        if (p) {
          this.authorization.populate(u, p);
          return callback(null, this.authorization);
        }
      }
    }
  ...
  signIn: function(callback) {
    ...
    this.authorization.create((err, a) => {
      ...
      if (atom.config.get("pull-request.useKeyring")) {
        keytar.replacePassword(KEYTAR_SERVICE, KEYTAR_USERNAME_KEY, a.username);
        keytar.replacePassword(KEYTAR_SERVICE, a.username, a.token);
      }
